...
import { FiStar, FiShoppingCart, FiHeart, FiShare2, FiChevronRight, FiMinus, FiPlus, FiBriefcase, FiCheck, FiAward, FiTruck } from 'react-icons/fi';
import { formatPrice } from '@/utils/priceFormatter';
import useCartStore from '@/store/useCartStore';
import { getProduct, getRelatedProducts, Product, ProductImage, ProductVariant } from '@/services/productService';
...
const mockReviews = [
  ...
  },
  {
    id: '2',
    product_id: '1',
    user: { name: 'Jane Doe', avatar: 'https://via.placeholder.com/50' },
    rating: 4,
    title: 'Good product, but...',
    comment: 'This product is good, but it could be better.',
    date: '2022-01-15',
    helpful_count: 5,
  },
];
      ...
      <div className="w-full bg-white dark:bg-gray-900 min-h-screen transition-colors duration-200">
        <div className="max-w-screen-2xl mx-auto px-4 py-8">
          <div className="bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-6 rounded-lg">
            ...
            <Link href="/products" className="mt-4 inline-block bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-md">
              Back to Products
            </Link>
          </div>
        </div>
            ...
            <div className="relative bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden h-96">
              <Image 
                src={product.images && product.images.length > 0 ? 
                  (activeImageIndex < product.images.length ? product.images[activeImageIndex].image : product.images[0].image) : 
                ...
                {reviews.map(review => (
                  <div key={review.id} className="border-b border-gray-200 dark:border-gray-700 pb-6">
                        ...
                        <div>
                          ...
                          <div className="flex text-yellow-400">
                            {[...Array(5)].map((_, i) => (
                              <FiStar 
                                key={i} 
                                className={`${i < review.rating ? 'fill-current' : ''} w-4 h-4`} 
                              />
                            ))}
                          </div>
                        </div>
                    ...
                    <button className="text-sm text-gray-500 dark:text-gray-400 flex items-center">
                      <FiHeart className="mr-1" /> Helpful ({review.helpful_count})
                    </button>
                  </div>
                ))}
              </div>
                ...
                <div key={relatedProduct.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                  <Link href={`/products/${relatedProduct.slug}`}>
                    <div className="relative h-48 bg-gray-100 dark:bg-gray-700">
                      <Image 
                        src={relatedProduct.images && relatedProduct.images.length > 0 ? relatedProduct.images[0].image : '/placeholder-image.jpg'} 
